...
const main = async () => {
  ...
  try {
    ...
    await octokit.paginate(
      ...
      (response) => issuesInMilestone.push(...response.data.map(issue => String(issue.number)))
    );
  } catch(error) {
  ...
  try {
    ...
    await octokit.paginate(
      ...
      listCardsParameters,
      (response) => {
        cardToIssueNumbers = response.data.filter(card => card.content_url != null).map(card => {
  ...
  core.info(`Found ${cardToIssueNumbers.length} cards in column (${GITHUB_DONE_COLUMN_ID})`)

  const cardsToArchive = cardToIssueNumbers.filter(tuple => issuesInMilestone.includes(tuple[1]))
  ...
  if (cardsToArchive.length > 0) {
    ...
    for (let i = 0; i < cardsToArchive.length; i += 1) {
      ...
      archive.push(octokit.projects.updateCard({
        card_id: cardId,
        archived: true,
      }));
    }
